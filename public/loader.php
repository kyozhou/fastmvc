...
$jsFiles = !empty($_GET['js']) ? $_GET['js'] : '';
$random = (int) (time() / 1);
if (!empty($jsFiles)) {
    $jsArray = explode('|', $_GET['js']);
    $tempFileName = md5($jsFiles . $random) . '.js';
    if (!file_exists($tempDir . '/' . $tempFileName)) {
        $content = '';
        foreach ($jsArray as $js) {
            if (file_exists($baseDir . '/' . $js)) {
                $content .= file_get_contents($baseDir . '/' . $js);
            }
        }
        $packer = new JavaScriptPacker($content, 'Numeric', true, false);
        $packed = $packer->pack();
//        $packed = $content;
        file_put_contents("$tempDir/$tempFileName", $packed);
    } else {
//        header('HTTP/1.1 304 Not Modified');
    }
//    header('HTTP/1.1 301 Moved Permanently');
//    header('location:'."$jsUrl/$tempFileName");
    echo file_get_contents($tempDir . '/' . $tempFileName);
}